...
const DateSlider: React.FC<DateSliderProps> = ({ categories }) => {
	...
	const [isSwiperOnBeginning, setIsSwiperOnBeginning] = useState(true);
	...
	//JSX
	console.log(swiperRef.current && swiperRef.current.slides.length - 1, currentDateIndex)
			...
			<div className="date-slider__container">
				...
				<div className="date-slider-mid">
					...
					<div className='date-slider-carousel'>
						...
						<CountUp start={((prevDate[0]) === undefined) ? (Number(currentDates[0].title)) : (Number(prevDate[0]))} end={Number(currentDate[0])} duration={0.8}>
							{
								({ countUpRef, start }) => (
									<span ref={countUpRef} className='date-slider-carousel__date1'>ategories[currentCategoryIndex].dates[0].title</span>
								)}
						</CountUp>
						<CountUp start={((prevDate[1]) === undefined) ? Number(currentDates[currentDates.length - 1].title) : Number(prevDate[1])} end={Number(currentDate[1])} duration={0.8}>
							{
								({ countUpRef, start }) => (
